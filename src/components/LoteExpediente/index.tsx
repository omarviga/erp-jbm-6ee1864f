...
import { useLote, useAnalisisRentabilidad } from '../../hooks/useLote';
...
const LoteExpediente: React.FC = () => {
  ...
  const { lote, loading, error } = useLote(loteId || '');
  const { analisis } = useAnalisisRentabilidad(lote?.id || '');
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant={
                  lote.estado === 'liquidado' ? "default" :
                    lote.estado === 'en_proceso' ? "secondary" :
                      "outline"
                }>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Columna Izquierda: Origen y Producción */}
        <div className="lg:col-span-2 space-y-6">
          <SeccionOrigen lote={lote} />
          <SeccionProduccion produccion={lote.produccion || []} />
        </div>

        {/* Columna Derecha: Cámara Fría y Rentabilidad */}
        <div className="space-y-6">
          <SeccionCamaraFria produccion={lote.produccion || []} />
          <SeccionRentabilidad
            lote={lote}
            analisis={analisis}
          />
      ...
      <div className="flex gap-2 justify-end pt-6">
        ...
        <Button onClick={() => alert(`Crear guía para ${lote.numero_lote}`)}>
