...
export function HuertosTab() {
  ...
  const [editingCortador, setEditingCortador] = useState<any>(null);
  ...
  const handleEditHuerto = (huerto: any) => {
  ...
  const createCortadorMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error al registrar cortador", description: error.message, variant: "destructive" });
    },
  });
  ...
  const handleEditCortador = (cortador: any) => {
  ...
  const handleToggleCortadorActivo = (cortador: any) => {
