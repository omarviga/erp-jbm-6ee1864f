...
type LoteReciente = Pick<
  Database["public"]["Tables"]["lotes"]["Row"],
  "id" | "numero_lote" | "fecha_recepcion" | "peso_neto" | "estado" | "es_cosecha_propia"
> & {
  productores: { nombre: string | null } | null;
  huertos: { nombre: string | null } | null;
};
...
export function RecentLotes() {
  ...
  const getOrigen = (lote: LoteReciente) => {
    if (lote.es_cosecha_propia && lote.huertos) {
      return `Cosecha Propia - ${lote.huertos.nombre || 'Sin huerto'}`;
    }
    if (lote.productores) {
      return lote.productores.nombre || 'Sin productor';
    }
    return "Sin origen";
  };
      ...
      <CardContent>
        <div className="space-y-3">
          {isLoading ? (
            ...
            (lotes as LoteReciente[]).map((lote) => (
              <div
                key={lote.id}
