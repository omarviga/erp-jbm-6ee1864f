...
type CamaraFriaTransferenciaItem = Database["public"]["Tables"]["camara_fria"]["Row"] & {
  produccion: {
    lote_id: string | null;
    calibre: string | null;
    calidad: string | null;
    ...
    lotes: { numero_lote: string | null } | null;
    presentaciones: { id: string | null; nombre: string | null } | null;
  } | null;
};
type ProduccionTransicionItem = Pick<
  Database["public"]["Tables"]["produccion"]["Row"],
  "id" | "destino" | "lote_id" | "calibre" | "calidad" | "cantidad_cajas" | "peso_total_kg" | "presentacion_id"
> & {
  lotes: { numero_lote: string | null } | null;
  presentaciones: { id: string | null; nombre: string | null } | null;
};

const invokeLooseRpc = async (fn: string, args: Record<string, unknown>) => {
...
const safeInsertKardex = async (payload: {
  ...
  try {
    const { error } = await supabase.from("inventario_kardex" as never).insert(payload as never);
...
export function useCrearTransferenciaCDMX() {
  ...
  const { data: stockDisponible = [], isLoading: loadingStock } = useQuery({
    ...
    queryFn: async () => {
      ...
      const desdeCamara = ((stockCamara || []) as CamaraFriaTransferenciaItem[]).map((item) => {
      ...
      const desdeTransicion = ((stockTransicion || []) as ProduccionTransicionItem[]).map((item) => ({
        id: item.id,
    ...
    mutationFn: async (datos: DatosTransferencia) => {
      ...
      for (const item of datos.items) {
        ...
        if (item.origen_inventario === "camara_fria") {
          const { error } = await invokeLooseRpc("registrar_envio_cdmx", {
        ...
        const { error } = await invokeLooseRpc("registrar_envio_cdmx_transporte_directo", {
