...
type ProcesarRecepcionTransferenciaResult = Database["public"]["Functions"]["procesar_recepcion_transferencia"]["Returns"][number];
type InventarioCDMXRow = Database["public"]["Tables"]["inventario_bodega_cdmx"]["Row"] & {
  presentacion: {
    nombre: string | null;
    peso_kg: number | null;
    tipo: string | null;
  ...
  const procesarRecepcion = useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await supabase.rpc("procesar_recepcion_transferencia", {
        ...
        p_detalles: detalles as unknown as Json,
        p_recibido_por: user.user.id,
      });

      if (error) throw error;
      return (data || []) as ProcesarRecepcionTransferenciaResult[];
    },
    onSuccess: (data) => {
...
export function useInventarioCDMX() {
  ...
  const { data: inventario, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return (data || []) as InventarioCDMXRow[];
    },
  });
