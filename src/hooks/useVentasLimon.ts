...
type PresentacionRow = Database["public"]["Tables"]["presentaciones"]["Row"];
type PresentacionLimonRow = PresentacionRow & {
    descripcion?: string | null;
    precio_base?: number | null;
};
type CamaraFriaStockRow = {
    cantidad_disponible: number;
    produccion: Pick<Database["public"]["Tables"]["produccion"]["Row"], "presentacion_id"> | null;
};
type VentaRow = Database["public"]["Tables"]["ventas"]["Row"];
type ProcessSaleResult = string | { id?: string } | { id?: string }[];
type ErrorLike = {
    message?: string;
};
...
const extraerVentaId = (ventaData: ProcessSaleResult): string | null => {
    if (typeof ventaData === 'string') return ventaData;
    if (Array.isArray(ventaData)) return ventaData[0]?.id || null;
    return ventaData?.id || null;
};
...
export function useVentasLimon() {
    ...
    const cargarStockLimon = useCallback(async () => {
        try {
        ...
        `);
            ...
            (data as CamaraFriaStockRow[] | null)?.forEach((item) => {
    ...
    const cargarProductosLimon = useCallback(async () => {
        try {
            setLoading(true);
            ...
            const productosLimon: ProductoLimon[] = (presentaciones || []).map((presentacion: PresentacionLimonRow) => {
    ...
    const cobrar = async (clienteId: string | null, montoRecibido: number, metodoPago: string) => {
        ...
        try {
            ...
            const ventaId = extraerVentaId(ventaData as ProcessSaleResult);
            if (!ventaId) {
                throw new Error("No se pudo identificar la venta registrada");
            }
