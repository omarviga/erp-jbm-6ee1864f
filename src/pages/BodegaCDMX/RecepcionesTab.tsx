...
export default function RecepcionesTab() {
  ...
  const actualizarDetalle = (
    ...
    campo: keyof DetalleRecepcion,
    valor: DetalleRecepcion[keyof DetalleRecepcion]
  ) => {
  ...
  // Initialize details with the shipped quantity visible to the operator.
  useEffect(() => {
    if (detalles && detalles.length > 0 && Object.keys(detallesRecepcion).length === 0) {
      ...
      detalles.forEach(d => {
        nuevosDetalles[d.presentacion_id] = {
          ...
          cantidad_recibida: d.cantidad_enviada,
          precio_base: d.precio_base,
          precio_venta: d.precio_venta ?? d.precio_base,
          notas_diferencia: '',
        };
      });
  ...
  const procesarRecepcionClick = async () => {
    ...
    const hayDiscrepanciaSinFoto = Object.values(detallesRecepcion).some(d => {
      const diferencia = (d.cantidad_recibida ?? 0) - d.cantidad_enviada;
    ...
    try {
      ...
      limpiarRecepcionActual();
    } catch (error: unknown) {
      const message = error && typeof error === "object" && "message" in error
        ? String((error as ErrorLike).message || "No se pudo completar la recepción.")
        : "No se pudo completar la recepción.";
      console.error("Error procesando recepción CDMX:", error);
  ...
  const calcularDiferencia = (presentacionId: string) => {
    ...
    if (!detalle) return 0;
    return (detalle.cantidad_recibida ?? 0) - detalle.cantidad_enviada;
  };
  ...
  const todosContados = () => {
    return Object.values(detallesRecepcion).every(
      d => d.cantidad_recibida !== null && d.cantidad_recibida >= 0
    );
  };
  ...
  return (
    <div className="h-full flex flex-col p-6 overflow-auto bg-background">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Recepción operativa de mercancía enviada desde Michoacán</p>
      </div>
      ...
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Confirma lo enviado, registra lo recibido y documenta cualquier diferencia con evidencia.
            </DialogDescription>
          </DialogHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-center">Cantidad Enviada</TableHead>
                ...
                <TableBody>
                  {detalles.map((d) => {
                    ...
                    const cantidadRecibida = detallesRecepcion[d.presentacion_id]?.cantidad_recibida;
                    const diferencia = (cantidadRecibida ?? 0) - d.cantidad_enviada;
                    const hayDiferencia = cantidadRecibida !== null && diferencia !== 0;

                    return (
                      <TableRow 
                        key={d.id}
                        className={cn(
                          !precioVentaValido && "bg-yellow-50",
                          hayDiferencia && "bg-rose-50/60"
                        )}
                      >
                        ...
                        <TableCell className="text-center">
                          ...
                          >
                            {cantidadRecibida === null ? "Pendiente" : `${diferencia > 0 ? "+" : ""}${diferencia}`}
                          </Badge>
                        </TableCell>
