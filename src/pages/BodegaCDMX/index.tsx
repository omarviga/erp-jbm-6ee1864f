...
const TABS: TabConfig[] = [
  { id: "pos", label: "Punto de Venta", icon: ShoppingCart, canAccess: ({ isAdmin, hasRole }) => isAdmin || hasRole("ventas") },
  { id: "recepciones", label: "Recepciones", icon: Truck, canAccess: ({ isAdmin, hasRole }) => isAdmin || hasRole("almacen") },
  { id: "inventario", label: "Inventario", icon: Package, canAccess: ({ isAdmin, hasRole }) => isAdmin || hasRole("almacen") || hasRole("ventas") },
  { id: "corte", label: "Corte de Caja", icon: Calculator, canAccess: ({ isAdmin, hasRole }) => isAdmin || hasRole("ventas") || hasRole("almacen") },
  { id: "gastos", label: "Gastos Locales", icon: Receipt, canAccess: ({ isAdmin, hasRole }) => isAdmin || hasRole("finanzas") },
  { id: "dashboard", label: "Rentabilidad", icon: BarChart3, canAccess: ({ isAdmin }) => isAdmin },
];
...
export default function BodegaCDMX() {
  ...
  const visibleTabs = TABS.filter((tab) => tab.canAccess({ isAdmin, hasRole }));
  const [activeTab, setActiveTab] = useState<TabId>(visibleTabs[0]?.id ?? "inventario");

  const canAccessTab = useCallback(
    (tabId: TabId) => visibleTabs.some((tab) => tab.id === tabId),
    [visibleTabs],
  );

  useEffect(() => {
    if (!canAccessTab(activeTab) && visibleTabs[0]) {
    ...
    <div className="h-full flex items-center justify-center bg-background p-6">
      <div className="max-w-md rounded-xl border bg-white p-6 text-center shadow-sm">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Tu usuario no tiene permisos para esta sección del módulo Bodega CDMX.
        </p>
      </div>
    </div>
  );
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "gastos":
        return <GastosTab />;
      case "dashboard":
        return isAdmin ? <DashboardTab /> : renderUnauthorized();
      default:
        return renderUnauthorized();
    }
  };
