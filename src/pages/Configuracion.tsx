...
export default function Configuracion() {
  ...
  const [editingPresentacion, setEditingPresentacion] = useState<PresentacionRow | null>(null);
  ...
  const { data: presentaciones, isLoading: loadingPresentaciones } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []) as PresentacionRow[];
    },
  });
  ...
  const createMutation = useMutation({
    ...
    onError: (error: unknown) => {
      toast({ title: "Error al crear presentación", description: (error as ErrorLike)?.message || "Error desconocido", variant: "destructive" });
    },
  });
  ...
  const handleEdit = (presentacion: PresentacionRow) => {
  ...
  const handleToggleActiva = (presentacion: PresentacionRow) => {
        ...
        <TabsContent value="camara">
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <p className="text-sm text-muted-foreground">
                Estas opciones se muestran como referencia y estarán disponibles para edición más adelante.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
