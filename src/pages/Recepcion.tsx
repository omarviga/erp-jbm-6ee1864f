...
export default function Recepcion() {
  ...
  const calculos = useMemo(() => {
    ...
    // 2. Kilos de Basura/Defecto (solo informativo, no descuenta pago)
    const porcentajeMerma = calidad.defectos || 0;
    const kilosMerma = netoFisico * (porcentajeMerma / 100);

    // 3. Peso a pagar: por regla operativa siempre es el peso neto.
    const pesoPagable = netoFisico;
  ...
  const huertoSeleccionado = useMemo(() => {
    if (!huertosDB || !huertoId) return undefined;
    return huertosDB.find((h) => h.id === huertoId);
  }, [huertoId, huertosDB]);
      ...
      <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {calidad.defectos > 0 && (
                        <div className="text-xs text-rose-600 font-medium text-center animate-pulse">
                          Merma registrada: {calculos.kilosMerma} kg ({calidad.defectos}%)
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <CardContent className="pt-6 space-y-6">
              {resumenVacio ? (
                <div className="rounded-xl border border-dashed border-white/15 bg-white/5 p-5 text-sm text-white/70">
                  Captura productor, pesos y precio para ver aquí el resumen de la recepción.
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-white/60">Folio</p>
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-white/60">Peso neto:</span>
                    ...
                    {calidad.defectos > 0 && (
                      <div className="flex justify-between text-sm text-rose-600">
                        <span>Merma registrada ({calidad.defectos}%):</span>
        ...
        <section className="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm">
          ...
          <div className="mx-auto w-full max-w-[360px] rounded-xl bg-slate-100 p-4 shadow-inner">
            <div ref={ticketPrintRef} className="ticket-print-root mx-auto w-[302px] bg-white px-4 py-3 font-mono text-[11px] leading-5 text-slate-900 shadow-lg">
              <div className="border-b border-dashed border-slate-300 pb-2 text-center">
                <img
                  src="/logo-ticket.png"
                  alt="Logo JBM"
                  className="mx-auto mb-1.5 h-16 w-auto object-contain"
                />
              ...
              <div className="mt-5 grid grid-cols-2 gap-4 text-center text-[9px]">
                <div>
                  <div className="border-t border-slate-500 pt-2 tracking-[0.15em]">FIRMA OPERADOR</div>
                </div>
                <div>
                  <div className="border-t border-slate-500 pt-2 tracking-[0.15em]">FIRMA PRODUCTOR</div>
                </div>
              </div>

              <p className="mt-4 text-center text-[10px] font-bold tracking-[0.2em]">GRACIAS POR SU PREFERENCIA</p>
            </div>
          </div>
