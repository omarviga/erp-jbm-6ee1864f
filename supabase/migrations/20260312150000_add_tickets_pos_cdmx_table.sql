-- Bitácora separada para tickets POS de Bodega CDMX.

CREATE TABLE public.tickets_pos_cdmx (
...
CREATE INDEX idx_tickets_pos_cdmx_created_at ON public.tickets_pos_cdmx(created_at DESC);
CREATE INDEX idx_tickets_pos_cdmx_numero_venta ON public.tickets_pos_cdmx(numero_venta);

ALTER TABLE public.tickets_pos_cdmx ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin y ventas pueden ver tickets POS CDMX"
ON public.tickets_pos_cdmx FOR SELECT
USING (
  public.has_role(auth.uid(), 'admin'::app_role)
  OR public.has_role(auth.uid(), 'ventas'::app_role)
);

CREATE POLICY "Solo funciones pueden insertar tickets POS CDMX"
ON public.tickets_pos_cdmx FOR INSERT
WITH CHECK (false);
...
BEGIN
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'nombre', vd.descripcion,
        'cantidad', vd.cantidad,
        'precio_venta', vd.precio_unitario,
  ...
  INSERT INTO public.tickets_pos_cdmx (
    venta_id,
    numero_venta,
    cliente_id,
    cliente_nombre,
    ...
    v_numero_venta,
    v_cliente_id_final,
    COALESCE(v_cliente_nombre, 'Cliente sin nombre'),
    p_metodo_pago::forma_pago,
    p_monto_total,
    v_ticket_items,
    auth.uid()
...
COMMENT ON TABLE public.tickets_pos_cdmx IS 'Bitácora separada de tickets emitidos por POS CDMX';
